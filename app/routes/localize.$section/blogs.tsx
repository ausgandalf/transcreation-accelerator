...
export const ResourcePanel = (props:ResourcePanelProps) => {
  ...
  const selectResource = (item) => {
    ...
    onSelect(item, searchParamValues, 'blog');
  }
  ...
  useEffect(() => {
    ...
    if (!fetcher.data) {
      ...
      if (fetcher.data.action == 'blog_list') {
        
        setKnownTotalPage(fetcher.data.total);
        if (fetcher.data.blogs.pageInfo.hasNextPage) {
          setCursor(fetcher.data.blogs.pageInfo.endCursor);
        } else {
          setCursor(''); // Disable loading
        }
        
        const newResources = [...resources, ...fetcher.data.blogs.nodes];
        // console.log('fetched', newResources);
        setResources(newResources);

        // Select first resource, if nothing selected.
        if (!selectedResource && (fetcher.data.blogs.nodes.length > 0)) selectResource(fetcher.data.blogs.nodes[0]);
  ...
  useEffect(() => {
    ...
    if (isFirstLoad.current || shouldLoad) doLoadBlogs();
