...
export type TranslationStatus = "in_progress" | "confirmed" | "needs_attention" | "not_translated";

// Get translation state by id, key, locale, market
export const getTranslationState = async (shop: string, resourceId: string, field: string, locale: string, market: string) => {
...
export const getTranslationStateByResourceId = async (shop: string, resourceId: string, locale: string, market: string) => {
  const params = new URLSearchParams({
...
export const updateTranslationState = async (
  ...
  console.log("formData", formData.getAll('previousValue'));  
  try {
...
export const deleteTranslationState = async (shop: string, resourceId: string, field: string, locale: string, market: string) => {
  const formData = new FormData();
